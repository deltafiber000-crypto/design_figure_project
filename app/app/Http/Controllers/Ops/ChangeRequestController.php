...
final class ChangeRequestController extends Controller
{
    public function show(int $id, SvgRenderer $renderer)
    {
        ...
        return view('admin.change-requests.show', [
            ...
            'baseSnapshotPdfUrl' => route('ops.change-requests.snapshot-base.pdf', $req->id),
            'compareSnapshotPdfUrl' => route('ops.change-requests.snapshot-compare.pdf', $req->id),
            'memoUpdateUrl' => route('ops.change-requests.memo.update', $req->id),
        ]);
    }
    ...
    public function updateMemo(\Illuminate\Http\Request $request, int $id)
    {
        ...
        return redirect()->route('ops.change-requests.show', $id)->with('status', 'リクエストメモを更新しました');
    }
